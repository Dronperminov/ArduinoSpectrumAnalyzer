...
const
  port_name = 'COM8'; {имя последовательного порта}
  port_speed = 115200; {скорость последовательного порта}
  
  screen_width = 810; {ширина графического окна}
  ...
  font_color = rgb(250, 250, 250); // цвет фона
  text_color = rgb(0, 0, 255); // цвет текста
  ...
  line_color = text_color; // цвет линий
...
begin  
  ...
  SetWindowCaption('Анализатор спектра сигнала с генератором сигналов');
  ...
  if generatorOnCheckBox.Checked then
    TextOut(zero_x + length_x div 2 - 15 * margin_x, zero_y - length_y - text_height - 10, 'Генератор включён');
...
begin
   ...
   if (freq < 5) or (freq > 23000) then begin
     stopGenerator;
     MessageBox.Show('Частота должна быть от 5 Гц до 23 кГц', 'Ошибка', MessageBoxButtons.OK, MessageBoxIcon.Warning);
     frequencyTextBox.Focus;
     exit;
   end;  
...
procedure nWaveParameterChanged(o:Object; e: EventArgs);
var
   logStr:string;
   log:real;
   
begin
   logStr := nWaveTextBox.Text;
   
   if logStr = '' then begin
     MessageBox.Show('Пожалуйста, укажите кол-во полос', 'Ошибка', MessageBoxButtons.OK, MessageBoxIcon.Warning);
     nWaveTextBox.Focus;
     exit;
   end;
   
   try
     log := StrToFloat(logStr);
   except 
      on System.FormatException do begin      
        MessageBox.Show('Кол-во должно быть целым числом', 'Ошибка', MessageBoxButtons.OK, MessageBoxIcon.Warning);
        nWaveTextBox.Focus;
        exit;
    end;
  end;
   
   if (log < 4) or (log > 100) then begin
     stopGenerator;
     MessageBox.Show('Кол-во полос может быть от 4 до 100 полос', 'Ошибка', MessageBoxButtons.OK, MessageBoxIcon.Warning);
     nWaveTextBox.Focus;
     exit;
   end;
   
   if (log < 6) and (log >= 4) then
     port.Write(chr(10) + 'log_base ' +  floatToStr( Exp(1 / log * Ln(2048)) + 2.5) + chr(10))
   else if (log >= 6) and (log < 9) then
     port.Write(chr(10) + 'log_base ' +  floatToStr( Exp(1 / log * Ln(2048)) + 0.5) + chr(10))
   else if log = 9 then
     port.Write(chr(10) + 'log_base 2.56' + chr(10))
   else if (log > 9) and (log < 13) then
     port.Write(chr(10) + 'log_base ' +  floatToStr( Exp(1 / log * Ln(2048)) + 0.1) + chr(10))
   else
     port.Write(chr(10) + 'log_base ' +  floatToStr( Exp(1 / log * Ln(2048)) - 0.02) + chr(10));
end;

procedure btnShowClick(o: Object; e: EventArgs);
begin
   nWaveParameterChanged(o, e);
end;
...
begin
   ...
   generatorOnCheckBox.Text := 'Генератор';
   ...
   frequencyLabel.Text := 'Частота, Гц:';
   ...
   waveformLabel.Text := 'Форма сигнала:';
   ...
   waveformComboBox.Items.Add('harmonic');
   ...
   nWaveLabel := new TLabel();
   nWaveLabel.Text := 'Кол-во полос:';
   nWaveLabel.ForeColor := System.Drawing.Color.Black;
   nWaveLabel.Location := new Point(0, 185);
   nWaveLabel.AutoSize := true;
   controlPanel.Controls.Add(nWaveLabel);
   
   nWaveTextBox := new TextBox();
   nWaveTextBox.Multiline := false;
   nWaveTextBox.Size := new Size(70, 25);
   nWaveTextBox.Location := new Point(0, 210);
   nWaveTextBox.Text := '40';
   controlPanel.Controls.Add(nWaveTextBox);
   
   btnShow := new Button();
   btnShow.Text := 'Показать';
   btnShow.Location := new Point(0, 240);
   btnShow.Size := new Size(70, 25);
   btnShow.Click += btnShowClick;
   controlPanel.Controls.Add(btnShow);
   ...
   controlPanel.Location := new Point(screen_width, 10); // расположение панели управления
   controlPanel.Size := new Size(140, screen_height); // размер панели управления
   
   MainForm.Controls.Add(controlPanel); // добавляем на главную форму панель управления
   
   MainForm.MinimumSize := new Size(screen_width + 100, screen_height); // размер главной формы
   
   graphABCControl.Size := new Size(screen_width - 10, screen_height); // размер графической части
...
begin
  if Setup(port) then begin
    port.Write(chr(10) + 'log_base 1.19' + chr(10));
    SendGenerator(port, 'stop', 0);
