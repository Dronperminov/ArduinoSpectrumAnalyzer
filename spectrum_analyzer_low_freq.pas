...
uses graphAbc, system.IO.Ports, system, system.IO;

const
  port_name = 'COM3'; {имя последовательного порта}
  port_speed = 115200; {скорость последовательного порта}
  
  screen_width = 400; {ширина графического окна}
  screen_height = 200; {высота графического окна}
...
procedure StartGenerator(var port:Serialport; waveform:string; freq:integer);
begin
...
begin
  if Setup(port) then begin
    StartGenerator(port, 'sin', 300);
    //port.Write(chr(10) + chr(10) + 'sin 100' + chr(10) + chr(10));
